...
- [x] 允许 segwit 仅带 NonWitnessUtxo
- [x] 从前序交易取 pkScript/value（并校验 txid/vout）

### 8. Finalization 后字段清理不一致

- [x] 统一"final 后尽量最小化"策略
- [x] 清理 PartialSigs/BIP32/TapScriptStack
- [x] 配置项控制是否保留 WitnessScript/RedeemScript/UTXO

### 9. Taproot 数据建模不够 PSBT-371 友好

- [x] 区分 tap_key_sig 和 tap_script_sig
- [x] 引入独立字段避免冲突（Input.TapKeySig / Input.TapScriptSigs）
- [x] 支持<xonlyPubKey, leafhash>键值

### 10. P2WPKH 最终化过度依赖 BIP32 条目

- [x] 回退用 PartialSigs 的 key 直接获取 pubkey
- [x] 支持缺少 BIP32 时的 finalize

### 11. Key 命名与类型

- [x] 统一入参为 pubkey []byte
- [x] 按脚本类型校验长度（33 压缩或 32 x-only）
...
- [x] 输出每个输入缺少的字段
- [x] 类似 Core 的 analyzepsbt 功能
- [x] 生成详细报告

### 13. 在 SignInput 存档 SighashType

- [x] 调用时传入的 sighash 存回 in.SighashType
- [x] 便于后续合并/审计
