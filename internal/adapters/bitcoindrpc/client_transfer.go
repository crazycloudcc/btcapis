...
func (c *Client) TransferAllToNewAddress(
	...
	fromAddrInfo, err := c.AddressValidate(ctx, fromAddress)
	if err != nil {
	...
	toAddrInfo, err := c.AddressValidate(ctx, toAddress)
	if err != nil {
	...
	utxos, err := c.AddressGetUTXOs(ctx, fromAddress)
	if err != nil {
	...
	feeRate, err := c.ChainEstimateSmartFeeRate(ctx, 6)
	if err != nil {
	...
	if feeRate.Feerate == 0 {
		feeRate.Feerate = 1.0 // 默认1 sat/vB
		logger.Warn("  ! 节点返回费率为0，使用默认费率: %.2f sat/vB", feeRate.Feerate)
	} else {
		logger.Info("  ✓ 费率估算完成")
		logger.Info("    - 目标确认块数: %d", feeRate.Blocks)
		logger.Info("    - 费率: %.2f sat/vB", feeRate.Feerate)
	}

	// 6. 构建PSBT交易
	logger.Info("[步骤8] 构建PSBT交易")
	psbtPacket, estimatedFee, err := c.buildPSBT(ctx, utxos, toAddress, fromAddrInfo, totalBalance, feeRate.Feerate)
	if err != nil {
	...
	finalHex, err := c.TxFinalizePsbt(ctx, psbtBase64)
	if err != nil {
	...
	txid, err := c.TxBroadcast(ctx, rawTx)
	if err != nil {
...
func getAddressType(addrInfo *ValidateAddressDTO) string {
...
func verifyPrivKeyMatchAddress(wif *btcutil.WIF, address string, addrInfo *ValidateAddressDTO) error {
...
func (c *Client) buildPSBT(
	...
	utxos []UTXODTO,
	toAddress string,
	fromAddrInfo *ValidateAddressDTO,
	totalBalance int64,
	feeRate float64,
	...
	estimatedSize := estimateTxSize(len(utxos), 1, fromAddrInfo)
	...
	for i, utxo := range utxos {
		if err := addPSBTInput(ctx, c, packet, i, utxo, fromAddrInfo); err != nil {
...
func estimateTxSize(numInputs, numOutputs int, addrInfo *ValidateAddressDTO) int {
...
func addPSBTInput(
	...
	if addrInfo.IsWitness || addrInfo.IsScript {
		...
		prevTxRaw, err := c.TxGetRaw(ctx, utxo.TxID, false)
		if err != nil {
...
func (c *Client) signPSBT(
	...
	for i := range utxos {
		...
		if addrInfo.IsWitness && addrInfo.WitnessVersion == 1 {
			...
			sigHash, err = c.getTaprootSigHash(packet, i, txscript.SigHashDefault)
		} else {
			// 其他类型使用传统或SegWit签名
			sigHashType := txscript.SigHashAll
			if addrInfo.IsWitness || addrInfo.IsScript {
				logger.Info("        * 使用SegWit签名方案")
				sigHash, err = c.getWitnessSigHash(packet, i, sigHashType)
			} else {
				logger.Info("        * 使用传统签名方案")
				sigHash, err = c.getLegacySigHash(packet, i, sigHashType)
			}
		}
...
func (c *Client) getTaprootSigHash(packet *psbt.Packet, inputIndex int, sigHashType txscript.SigHashType) ([]byte, error) {
...
func (c *Client) getWitnessSigHash(packet *psbt.Packet, inputIndex int, sigHashType txscript.SigHashType) ([]byte, error) {
...
func (c *Client) getLegacySigHash(packet *psbt.Packet, inputIndex int, sigHashType txscript.SigHashType) ([]byte, error) {
